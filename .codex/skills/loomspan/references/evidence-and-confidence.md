...
A `MODEL_ATTEMPT_FAILED` fact establishes one failed physical attempt, not a
terminal execution or a recovered retry. Use its normalized facts for compact
orientation, its exact `contentRef` for the ordered stack/provider diagnostic
value, and a later successful response in the same retry sequence as the
evidence of recovery. Progressive ranges limit presentation size; they add no
authorization, redaction, or retention guarantee.
...
For concurrency, preserve nullable assignments and nullable observed overlap.
Canonical sequence and completion order are recorded ordering facts, not
causal or importance judgments. Sequential-equivalent folding follows accepted
task order, including when concurrent work completed out of order.
