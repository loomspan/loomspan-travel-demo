...
function App(){
  ...
  const assessment=trip?.assessments.find(a=>a.revision===trip.revision);
  ...
  async function load(id:string){
    ...
    try{const value=await api<Trip>('/trips/'+id);if(version!==selectionVersion.current)return;setTrip(value);setDraft(value.request);setScreen(value.booking?'booked':value.assessments.some(a=>a.revision===value.revision)?'compare':'request');localStorage.setItem('wayfarer.trip',id);}
  ...
  useEffect(()=>{
    ...
    const timer=setInterval(()=>void poll(),2500);const clock=setInterval(()=>setTick(Date.now()),1000);
  ...
  async function saveAndAssess(event:React.FormEvent){
    ...
    try{
      ...
      else if(JSON.stringify(saved.request)!==JSON.stringify(draft))saved=await api<Trip>('/trips/'+saved.id,'PUT',{revision:saved.revision,request:draft});
  ...
  async function retry(){if(!trip)return;setBusy(true);setError('');setReview(null);try{await api('/trips/'+trip.id+'/assessments','POST',{revision:trip.revision});setTrip(await api<Trip>('/trips/'+trip.id));setTick(Date.now());}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
  async function book(){
    if(!trip||!review||!assessment)return;setBusy(true);setError('');
    try{await api('/trips/'+trip.id+'/bookings','POST',{assessmentId:assessment.id,candidateId:review.candidateId,idempotencyKey:bookingKey.current});setTrip(await api<Trip>('/trips/'+trip.id));setScreen('booked');setReview(null);await refreshList();}
    catch(e){setError((e as Error).message);}finally{setBusy(false);}
  }
  ...
  return <div className="app">
    ...
    <div className="shell"><aside><div className="aside-heading">YOUR TRIPS</div><button className="new-trip" onClick={newTrip} disabled={busy||running}>+ Plan a weekend</button>
      {trips.map((item,index)=><button disabled={busy} key={item.id} className={'trip-link '+(trip?.id===item.id?'selected':'')} onClick={()=>void load(item.id)}><span>New York weekend {trips.length-index}</span><small>Oct 16–18 · {item.booked?'Booked':'Planning'}</small></button>)}
      ...
      <div className="mobile-trips"><label>Saved trips<select value={trip?.id??''} disabled={busy} onChange={e=>e.target.value?void load(e.target.value):newTrip()}><option value="">New weekend</option>{trips.map((t,i)=><option key={t.id} value={t.id}>Weekend {trips.length-i} · {t.booked?'Booked':'Planning'}</option>)}</select></label></div>
      <nav aria-label="Trip steps" className="steps"><button aria-current={screen==='request'?'step':undefined} onClick={edit} disabled={busy||!!trip?.booking}>1 · Your request</button><button aria-current={screen==='compare'?'step':undefined} onClick={()=>setScreen('compare')} disabled={!assessment||busy||!!trip?.booking}>2 · Compare trips</button><button aria-current={screen==='booked'?'step':undefined} onClick={()=>setScreen('booked')} disabled={!trip?.booking}>3 · Booked trip</button></nav>
      {error&&<div className="notice error" role="alert">{error}</div>}
      {!draft&&!error&&<p role="status">Loading the trip workspace…</p>}
      {screen==='request'&&draft&&<form className="panel" onSubmit={saveAndAssess}><div className="row"><h2>A weekend that fits</h2><button type="button" onClick={()=>setDraft(example)} disabled={busy||running}>Load example</button></div>
        <p className="muted">This first demo supports the dates below, two adults, and one room. Adjust your budget, timing, and priorities.</p>
        <fieldset disabled={busy||running}><div className="fields">
          ...
          <label>Hotel-ready by Friday<input required type="time" value={draft.hotelReadyBy.slice(11,16)} onChange={e=>setField('hotelReadyBy',draft.outboundDate+'T'+e.target.value+':00-04:00')}/></label>
          <label>Leave hotel no earlier than Sunday<input required type="time" min="11:00" value={draft.leaveHotelNoEarlierThan.slice(11,16)} onChange={e=>setField('leaveHotelNoEarlierThan',draft.returnDate+'T'+e.target.value+':00-04:00')}/></label>
          <label>Back in Boston by Sunday<input required type="time" value={draft.returnToOriginBy.slice(11,16)} onChange={e=>setField('returnToOriginBy',draft.returnDate+'T'+e.target.value+':00-04:00')}/></label>
        </div><p className="small muted">{scope} The Boston meeting point is the rail station; airport transfers are included when needed.</p><button className="primary" type="submit">{busy?'Saving…':running?'Assessment running…':'Confirm and compare with Loomspan'}</button></fieldset>
      </form>}
      {screen==='compare'&&<>
        ...
        {proposal&&<>
          {proposal.status==='NO_FEASIBLE_TRIP'?<div className="notice" role="status">{proposal.blockers.map(v=><p key={v.code}>{v.message}</p>)}<p>No bookable proposal was created.</p></div>:<div className="options">
            {proposal.recommended&&<TripCard quote={proposal.recommended} label="Recommended" reason={proposal.recommendedReason} budget={trip!.request.budgetCents} primary onChoose={choose} disabled={busy}/>}
            {proposal.alternative&&<TripCard quote={proposal.alternative} label={proposal.recommended&&proposal.alternative.totalCents<proposal.recommended.totalCents?'Save '+money(proposal.recommended.totalCents-proposal.alternative.totalCents):'An alternative'} reason={proposal.alternativeReason} budget={trip!.request.budgetCents} onChoose={choose} disabled={busy}/>}
          </div>}
        ...
        {review&&<section className="panel review" aria-label="Review booking"><h2>Review your booking</h2><p>{review.hotelName} · {review.roomDescription}</p><p>{mode(review.outboundMode)} outbound · {mode(review.returnMode)} return · October 16–18 · Two adults</p><p className="price">{money(review.totalCents)} <span className="small">total</span></p><Itinerary quote={review}/><p className="small muted">This confirms a simulated booking against local inventory. Two seats per service and one room on each night will be reserved. Transfers are included as priced line items.</p><div className="actions"><button ref={reviewButton} className="primary" onClick={()=>void book()} disabled={busy}>{busy?'Confirming…':'Confirm simulated booking'}</button><button onClick={()=>setReview(null)} disabled={busy}>Back to comparison</button></div></section>}
      </>}
      {screen==='booked'&&trip?.booking&&<><span className="badge">Simulated booking confirmed</span><h2 className="booked-title">Your weekend is ready.</h2><p>{trip.booking.quote.hotelName} · {money(trip.booking.quote.totalCents)} total</p><div className="panel"><Itinerary quote={trip.booking.quote}/><p className="small muted">Booking reference: <code>{trip.booking.id}</code></p><p className="small">Two seats per service · One room on October 16 and 17 · Transfers priced, not separately reserved</p></div><p className="small muted">Saved in the local database. This booking survives refresh and application restart.</p></>}
    </main></div><footer>Fictional services and prices · No live supplier bookings · Shared local demo workspace</footer>
  </div>;
}
function TripCard({quote:q,label,reason,budget,primary=false,onChoose,disabled}:{quote:Quote;label:string;reason:string|null;budget:number;primary?:boolean;onChoose:(q:Quote)=>void;disabled:boolean}){
  return <article className={'choice '+(primary?'recommended':'')}><span className="badge">{label}</span><h3>{q.hotelName}</h3><p className="small muted">{mode(q.outboundMode)} outbound · {mode(q.returnMode)} return</p><p>{q.roomDescription}</p><div className="price">{money(q.totalCents)}</div><div className="small muted">{money(budget-q.totalCents)} below your limit</div><dl><div><dt>Transport · Two travelers</dt><dd>{money(q.transportCents)}</dd></div><div><dt>Room · Two nights</dt><dd>{money(q.lodgingCents)}</dd></div><div><dt>Transfers · Entire trip</dt><dd>{money(q.transferCents)}</dd></div></dl><div>Hotel-ready Friday at {time(q.hotelReadyAt)}</div><div>{q.totalTransferMinutes} minutes of transfers across the trip</div><p className="reason">{reason}</p><details><summary>View itinerary</summary><Itinerary quote={q}/></details><button className={primary?'primary':''} disabled={disabled} onClick={()=>onChoose(q)}>Review {money(q.totalCents)} trip</button></article>;
}
function Itinerary({quote:q}:{quote:Quote}){
  return <div className="itinerary"><h3>Friday · Getting there</h3><ol><li><time>{time(q.startAt)}</time><span>Be at the Boston downtown meeting point{q.outboundMode==='flight'?' for your airport transfer':''}</span></li><li><time>{time(q.outboundDepartsAt)}</time><span>{mode(q.outboundMode)} departs Boston · {q.outboundServiceId}</span></li><li><time>{time(q.outboundArrivesAt)}</time><span>Arrive in New York · Arrival buffer and hotel transfer follow</span></li><li><time>{time(q.hotelArrivalAt)}</time><span>Arrive at {q.hotelName}</span></li><li><time>{time(q.hotelReadyAt)}</time><span>Room ready for check-in</span></li></ol><h3>Sunday · Heading back</h3><ol><li><time>11:00</time><span>Check out · Free luggage storage and lounge access</span></li><li><time>{time(q.leaveHotelAt)}</time><span>Leave the hotel for your return service</span></li><li><time>{time(q.returnDepartsAt)}</time><span>{mode(q.returnMode)} departs New York · {q.returnServiceId}</span></li><li><time>{time(q.returnArrivesAt)}</time><span>Arrive in Boston</span></li><li><time>{time(q.returnToOriginAt)}</time><span>Trip ends at the Boston downtown meeting point</span></li></ol></div>;
}
