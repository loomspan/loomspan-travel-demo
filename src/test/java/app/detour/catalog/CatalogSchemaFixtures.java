...
final class CatalogSchemaFixtures {
    ...
    static void ensureMinimumCatalog(JdbcTemplate jdbc) {
        ...
        if (jdbc.queryForObject("SELECT COUNT(*) FROM rental_unit WHERE catalog_key = 'test-car-unit-1'", Integer.class) == 0) {
            if (jdbc.queryForObject("SELECT COUNT(*) FROM catalog_supplier WHERE catalog_key = 'test-lodging'", Integer.class) == 0) {
                jdbc.update("INSERT INTO catalog_supplier (catalog_key, name, supplier_category) VALUES ('test-lodging', 'Test Lodging', 'LODGING')");
            }
            if (jdbc.queryForObject("SELECT COUNT(*) FROM accommodation_property WHERE catalog_key = 'test-property-sfo'", Integer.class) == 0) {
            ...
            if (jdbc.queryForObject("SELECT COUNT(*) FROM catalog_supplier WHERE catalog_key = 'test-cars'", Integer.class) == 0) {
                jdbc.update("INSERT INTO catalog_supplier (catalog_key, name, supplier_category) VALUES ('test-cars', 'Test Cars', 'CAR_RENTAL')");
            }
            if (jdbc.queryForObject("SELECT COUNT(*) FROM rental_vehicle_class WHERE catalog_key = 'test-car-standard'", Integer.class) == 0) {
                jdbc.update("INSERT INTO rental_vehicle_class (catalog_key, supplier_id, supplier_category, rental_location_id, vehicle_category, name, daily_base_price_cents, daily_tax_cents, daily_fee_cents) VALUES ('test-car-standard', (SELECT id FROM catalog_supplier WHERE catalog_key = 'test-cars'), 'CAR_RENTAL', (SELECT id FROM rental_location WHERE catalog_key = 'rental-location-sfo'), 'STANDARD', 'Test car', 1, 0, 0)");
            }
